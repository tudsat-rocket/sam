use egui::{DragValue, Ui, InnerResponse};
    ...
    fn ui(&mut self, ui: &mut Ui) -> InnerResponse<()> {
        egui::Grid::new("simulation_settings_grid")
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.weak("Altitude");
                    ui.add(DragValue::new(&mut self.altitude_ground)
                           .suffix(" m")
                           .speed(1.0)
                           .clamp_range(-100.0..=2000.0));

                    ui.weak("Angle");
                    ui.add(DragValue::new(&mut self.launch_angle)
                           .suffix(" °")
                           .speed(0.5)
                           .clamp_range(0.5..=90.0));

                    ui.weak("Azimuth");
                    ui.add(DragValue::new(&mut self.launch_azimuth)
                           .suffix(" °")
                           .speed(1.0)
                           .clamp_range(-360.0..=359.0));
                });
                ui.end_row();
                ...
                ui.horizontal(|ui| {
                    ui.weak("Latitude");
                    ui.add(DragValue::new(&mut self.launch_latitude)
                           .suffix(" °")
                           .speed(0.000001)
                           .clamp_range(-90.0..=90.0));

                    ui.weak("Longitude");
                    ui.add(DragValue::new(&mut self.launch_longitude)
                           .suffix(" °")
                           .speed(0.000001)
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add(DragValue::new(&mut self.sim_start_delay)
                           .suffix(" ms")
                           .speed(100)
                           .clamp_range(5001..=50000));
                });
                ui.end_row();

                ui.label("Thrust");
                ui.horizontal(|ui| {
                    ui.add(DragValue::new(&mut self.thrust)
                           .suffix(" m/s²")
                           .speed(1.0)
                           .clamp_range(10.0..=1000.0));

                    ui.weak("for");
                    ui.add(DragValue::new(&mut self.thrust_duration)
                           .suffix(" ms")
                           .speed(10)
                ...
                ui.horizontal(|ui| {
                    ui.weak("Flight");
                    ui.add(DragValue::new(&mut self.drag_flight)
                           .suffix("")
                           .speed(0.001)
                           .clamp_range(0.0..=500.0));

                    ui.weak("Drogue");
                    ui.add(DragValue::new(&mut self.drag_drogue)
                           .suffix("")
                           .speed(0.001)
                           .clamp_range(0.0..=500.0));

                    ui.weak("Main");
                    ui.add(DragValue::new(&mut self.drag_main)
                           .suffix("")
                           .speed(0.01)
                           .clamp_range(0.0..=500.0));
                });
