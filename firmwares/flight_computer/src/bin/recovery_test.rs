...
use flight_computer_firmware::BoardAdc;
use flight_computer_firmware::BoardOutputs;
use lora_phy::LoRa;
...
async fn main(low_priority_spawner: Spawner) {
    ...
    // low_priority_spawner.spawn(flight_computer_firmware::vehicle::run(vehicle, board.iwdg)).unwrap();
    low_priority_spawner.spawn(run_recovery_test(board.outputs)).unwrap();
