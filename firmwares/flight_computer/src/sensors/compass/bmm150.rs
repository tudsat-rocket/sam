...
impl<SPI: SpiBus, CS: OutputPin> BMM150<SPI, CS> {
    ...
    fn read_registers(&mut self, address: BMM150Register, response_len: usize) -> Result<Vec<u8>, SPI::Error> {
        free(|cs| {
    ...
    fn write_u8(&mut self, address: BMM150Register, value: u8) -> Result<(), SPI::Error> {
        free(|cs| {
            let mut ref_mut = self.spi.borrow(cs).borrow_mut();
            let spi = ref_mut.deref_mut();
    ...
    fn read_trim_data(&mut self) -> Result<(), SPI::Error> {
    ...
    fn set_power_control(&mut self, power_control: bool) -> Result<(), SPI::Error> {
        self.write_u8(BMM150Register::PowerControl, power_control as u8)
    }

    fn configure_operation(&mut self, data_rate: BMM150DataRate, opmode: BMM150OpMode) -> Result<(), SPI::Error> {
