...
use crate::animations::YELLOW;
use crate::animations::{
    self, AnimArgs, ORANGE,
    helpers::{mk_every_nth, mk_fill_percentage},
    ping_pong,
};
...
async fn boot_animation(
    ...
    for i in 0..buffer.len() {
        mk_fill_percentage(((100 * i) / buffer.len()) as u8, color, buffer, Some(BLACK.into()));
        update_cb(buffer);
        Timer::after(led_lightup_time).await;
    }
    Timer::after(Duration::from_millis(200)).await;
    mk_fill_percentage(100, BLACK.into(), buffer, Some(BLACK.into()));
    update_cb(buffer);
    Timer::after(Duration::from_millis(200)).await;
}
...
//             match time_ms % 1000 {
//                 0..250 => mk_every_nth(4, 0, n2_filling, buffer, Some(BLACK)),
//                 250..500 => mk_every_nth(4, 1, n2_filling, buffer, Some(BLACK)),
//                 500..750 => mk_every_nth(4, 2, n2_filling, buffer, Some(BLACK)),
//                 750..1000 => mk_every_nth(4, 3, n2_filling, buffer, Some(BLACK)),
//
...
//             mk_fill_percentage(100, BLACK, buffer, None);
//             match time_ms % 1000 {
//                 0..250 => mk_every_nth(4, 0, n2o_filling, buffer, Some(BLACK)),
//                 250..500 => mk_every_nth(4, 1, n2o_filling, buffer, Some(BLACK)),
//                 500..750 => mk_every_nth(4, 2, n2o_filling, buffer, Some(BLACK)),
//                 750..1000 => mk_every_nth(4, 3, n2o_filling, buffer, Some(BLACK)),
//
//                 _ => (),
//             }
//         }
...
pub async fn run(
    ...
    let mut buffer: [RGB8; 20] = [BLACK.into(); 20];

    boot_animation(&mut buffer, Duration::from_millis(20), GREEN.into(), |b: &[RGB8]| {
    ...
    // let test_anim = AnimArgs::new_simple(4000, RED).unwrap();
    let test_anim = AnimArgs::new(4000, 0, RED, Some(GREEN)).unwrap();
    loop {
        ping_pong(Instant::now().as_millis() as u32, &mut buffer, &test_anim, 20, |b: &[RGB8]| {
            let _ = leds.write(b.iter().copied());
        });
