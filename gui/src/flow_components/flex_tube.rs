...
use super::{
    constants::{FLEX_TUBE_HEIGHT, FLEX_TUBE_STEP_NUM, FLEX_TUBE_TICK_LENGTH, FLEX_TUBE_TICK_NUM, STROKE_WITH},
...
impl ComponentPainter for FlexTubePainter {
    fn paint(&self, ui: &mut egui::Ui) -> ResponseBounds {
        ...
        let path_tube: Vec<_> = (0..=FLEX_TUBE_STEP_NUM)
            ...
            .map(|r| {
                pos + Vec2::new(r * width / (2.0 * std::f32::consts::PI) - width / 2.0, -r.sin() * tube_half_height)
            })
            .collect();
        ...
        for i in 1..(FLEX_TUBE_TICK_NUM + 1) {
            ...
            painter.line(
                vec![
                    mid + Vec2::new(0.0, tick_half_length),
                    mid - Vec2::new(0.0, tick_half_length),
                ],
                stroke,
            );
