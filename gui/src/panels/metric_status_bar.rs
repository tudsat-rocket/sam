...
fn display(metric: &Metric, backend: &Backend, metric_monitor: &MetricMonitor, ui: &mut Ui, theme :&ThemeColors) {
    ...
    let worst_constraint_result = metric_monitor.constraint_results().get(metric).map(|res| res.iter().max()).unwrap_or_default();

    worst_constraint_result.map(|constraint| { 
...
const METRIC_MONITOR_FILTER_BUTTON_ID: LazyLock<egui::Id> = LazyLock::new(|| egui::Id::new("MetricMonitorFilterButton"));

impl MetricStatusBar {

    fn filter_constraints<'a>(constraint_results: &'a HashMap<Metric, Vec<EvaluatedConstraint>>, active_constraint_mask: &Vec<ConstraintResult>) -> HashMap<Metric, Vec<&'a EvaluatedConstraint>> {
        return constraint_results.iter().map(|(m, ecs)| {
        ...
        egui::SidePanel::left("Monitor Panel").show_animated(ctx, num_metrics_to_display > 0 || popup_manager.has_any_open_popup(&METRIC_MONITOR_FILTER_BUTTON_ID), |ui| {
            ...
            popup_manager.add_dropdown_menu(&trigger, dropdown_response.interact_rect.right_top(), ui, |ui, _popup_manager| {
                ...
                for constraint_result in ConstraintResult::iter() {
                    let active_position = active_constraint_mask.iter().find_position(|e| **e == constraint_result).map(|(i, _c)| i);
