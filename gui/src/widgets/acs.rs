use egui::{
    ...
    Color32, CornerRadius, Painter, Pos2, Rect, RichText, Shape, Stroke, Vec2,
};
...
const ACS_TANK_MAX_PRESSURE: f64 = 300.0;

pub struct AcsSystemDiagram {
    valve_status: Option<ThrusterValveState>,
    propellant_f: f32,
    tank_pressure: Option<f64>,
    regulator_pressure: Option<f64>,
}

impl AcsSystemDiagram {
    pub fn new(backend: &Backend) -> Self {
        let tank_pressure = backend.current_value(Metric::Pressure(telemetry::PressureSensorId::AcsTank));
        let regulator_pressure = backend.current_value(Metric::Pressure(telemetry::PressureSensorId::AcsPostRegulator));

        Self {
            valve_status: backend.current_enum(Metric::ThrusterValveState),
            propellant_f: (tank_pressure.unwrap_or_default() / ACS_TANK_MAX_PRESSURE) as f32,
            tank_pressure,
            regulator_pressure,
        }
    }
    ...
    fn draw_tank(
        ...
        let mut path_complete: Vec<_> =
            path_bulkhead.iter().map(|v| pos + Vec2::new(v.x, -height / 2.0 + v.y)).collect();
        path_complete.extend(path_bulkhead.iter().rev().map(|v| pos + Vec2::new(v.x, height / 2.0 - v.y)));

        let mut path_fill: Vec<_> =
            path_bulkhead.iter().rev().map(|v| pos + Vec2::new(v.x, height / 2.0 - v.y)).collect();
        ...
        path_fill = path_fill
            .into_iter()
            .filter(|v| (v.y - pos.y) > (height / 2.0 - height * self.propellant_f - 1.0))
            .collect();
    ...
    fn draw_regulator(&self, painter: &Painter, pos: Pos2, w: f32, stroke: Stroke) {
        ...
        painter.rect(
            Rect::from_center_size(pos, Vec2::splat(w)),
            0.0,
            Color32::TRANSPARENT,
            stroke,
    ...
    fn draw_valve(&self, painter: &Painter, pos: Pos2, w: f32, open: bool, vertical: bool, stroke: Stroke) {
        ...
        let c = if open { Color32::TRANSPARENT } else { stroke.color };
...
impl egui::Widget for AcsSystemDiagram {
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ...
        ui.vertical(|ui| {
            ...
            painter.vline(
                pos_nozzle_tee.x,
                pos_nozzle_valve_down.y - symbol_w / 2.0..=pos_nozzle_valve_up.y + symbol_w / 2.0,
                stroke,
            );
            painter.vline(
                pos_nozzle_tee.x,
                pos_nozzle_valve_down.y + symbol_w / 2.0..=pos_nozzle_down.y - symbol_w / 2.0,
                stroke,
            );
            painter.vline(
                pos_nozzle_tee.x,
                pos_nozzle_valve_up.y - symbol_w / 2.0..=pos_nozzle_up.y + symbol_w / 2.0,
                stroke,
            );
        })
        .response
