...
pub const CENTERED_UNIT_RECT: Parallelogram = Parallelogram { center: Point2::new(0f32, 0f32), axes: [Vector2::new(-0.5, 0.5), Vector2::new(0.5, 0.5)] };
...
impl Parallelogram {
    ...
    pub fn axis_aligned_bounding_box(&self) -> Rect {
        Rect::from_center_size(to_pos(self.center), Vec2::new(2f32 * self.axes[0].x.abs().max(self.axes[1].x.abs()), 2f32 * self.axes[0].y.abs().max(self.axes[1].y.abs())))
    }
...
impl From<Rect> for Parallelogram {
    ...
    fn from(rect: Rect) -> Self {
        Parallelogram { center: to_point(rect.center()), axes: [to_vector(rect.left_top() - rect.center()), to_vector(rect.right_top() - rect.center())] }
    }
