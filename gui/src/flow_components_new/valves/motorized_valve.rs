...
use crate::{flow_components_new::{core::{constants::STROKE_WIDTH, flow_painter::{Painter, Symbol}}, math::{conversions::to_pos, transform::Transform}}, utils::theme::ThemeColors};
...
pub fn paint(transform: &Affine2<f32>, state: &ValveState, painter: &egui::Painter, ctx: &egui::Context) {
    ...
    Symbol::new(Painter::Motor, Transform::new(Rotation2::identity(), Scale2::new(MOTOR_DIAMETER, MOTOR_DIAMETER), Translation2::new(0f32, -(HANDLE_LENGTH + MOTOR_DIAMETER/2f32)))).paint(transform, painter, ctx);
}