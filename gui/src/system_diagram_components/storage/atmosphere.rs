...
use crate::{system_diagram_components::{core::constants::STROKE_WIDTH, math::{conversions::to_pos, ellipse::Circle}, primitives::*}, utils::theme::ThemeColors};
...
pub fn paint(transform: &Affine2<f32>, painter: &egui::Painter, ctx: &egui::Context) {
    ...
    character::paint("\u{24B6}", transform * Point2::origin(), 2f32 * Circle::new(Point2::origin(), 0.5).transform(transform).incircle().radius(), painter, ctx);
}