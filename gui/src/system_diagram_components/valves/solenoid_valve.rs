...
use crate::{system_diagram_components::{core::constants::STROKE_WIDTH, math::{conversions::to_pos, transform::Transform}, sensors_and_actuators::solenoid}, utils::theme::ThemeColors};
...
const POSITIONS: [Point2<f32>; 6] = [
    Point2::new(0.5, -0.5), Point2::new(0.5, 0.5),          //Left
    Point2::new(-0.5, -0.5), Point2::new(-0.5, 0.5),        //Right
    Point2::new(0.0, 0.0), Point2::new(0.0, -HANDLE_LENGTH) //Handle
];
...
pub fn paint(transform: &Affine2<f32>, state: &ValveState, painter: &egui::Painter, ctx: &egui::Context) {
    ...
    let solenoid_transform = transform * Transform::new(Rotation2::identity(), Scale2::new(MOTOR_DIAMETER, MOTOR_DIAMETER), Translation2::new(0f32, -(HANDLE_LENGTH + MOTOR_DIAMETER/2f32))).to_affine2();
    let generic_valve_transform = transform * Transform::new(Rotation2::identity(), Scale2::new(0.8, 1f32), Translation2::identity()).to_affine2();
